...
def api_team_search(query: str):
    """Find a college football team by name, abbreviation, or nickname."""
    try:
        teams = get_all_teams()
        q = query.strip().lower()
...
def get_game_summary(event_id: str):
    """Return ESPN's full game summary, including situation and plays."""
    if not event_id:
...
def api_game(event_id: str):
    try:
        data = get_game_summary(event_id)
        return data
    except Exception as e:
        return JSONResponse(
            status_code=500,
            content={"error": str(e), "event_id": event_id}
        )
